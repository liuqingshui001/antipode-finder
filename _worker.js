...
<body>
    ...
    <script type="module">
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    doc.querySelectorAll('item').forEach(el => {
                        ...
                        if (title) items.push({ title, snippet: desc.replace(/<[^>]+>/g, '').substring(0, 120), url: link, date, source: 'Bing' });
                    });
                    if (items.length > 0) return items.slice(0, 6);
                }
            } catch { /* fall through */ }
            try {
                const resp = await fetchWithCORS(htmlUrl, { headers:{'User-Agent':'Mozilla/5.0'} });
                if (!resp.ok) return [];
                const html = await resp.text();
                const doc = new DOMParser().parseFromString(html, 'text/html');
                const items = [];
                doc.querySelectorAll('.news-card, .newsitem, .t_nw').forEach(el => {
                    const a = el.querySelector('a[href]');
                    const title = a ? a.textContent.trim() : '';
                    const link = a ? a.getAttribute('href') || '' : '';
                    const snippet = el.querySelector('.snippet, .abstract, .d-snippet')?.textContent?.trim() || '';
                    if (title) items.push({ title, snippet: snippet.substring(0, 120), url: link, date: '', source: 'Bing' });
                });
                return items.slice(0, 6);
            } catch { return []; }
        }

        /** Fetch related Wikipedia articles as "news" */
        async function fetchWikiNews(name, useCN) {
            const domain = useCN ? 'zh.wikipedia.org' : 'en.wikipedia.org';
            const url = \`https://\${domain}/w/api.php?action=query&list=search&srsearch=\${encodeURIComponent(name)}&srlimit=6&format=json&origin=*\`;
            try {
                const resp = await fetchWithCORS(url, { headers:{'User-Agent':'AntipodeFinder/1.0'} });
                if (!resp.ok) return [];
                const data = await resp.json();
                ...
                return results.map(r => ({
                    ...
                    url: \`https://\${domain}/wiki/\${encodeURIComponent(r.title)}\`,
        ...
        async function loadSidebarContent(antiName, antiLat, antiLng, rawAddress) {
            ...
            // Load news: Bing first, Wikipedia fallback (both source)
            let items = [];
            if (isChineseUser) {
            ...
            // If Bing failed, fall back to Wikipedia
            if (items.length === 0) {
                if (isChineseUser) {
                    items = await fetchWikiNews(bestName, true);
                    if (items.length === 0) items = await fetchWikiNews(bestName, false);
                } else {
                    items = await fetchWikiNews(bestName, false);
                    if (items.length === 0) items = await fetchWikiNews(bestName, true);
                }
            }
            ...
            if (items.length > 0) {
                tabNews.innerHTML = items.map(item => \`
                    <div class="news-item">
                        ...
                        <div class="news-meta">\${escapeHtml(item.snippet || item.date || 'Wikipedia')}</div>
                    </div>
                \`).join('');
            } else {
